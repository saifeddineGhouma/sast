...
jQuery(function($){
    ...
    PRICE FILTER
******************************************/
var from = <?php echo e($minPrice); ?>,
to = <?php echo e($maxPrice); ?>;
...
$("#amount_two").change(function() { $("#slider-range_one").slider('values',1,$(this).val()); });
	...
	function reload_products(){
	   ...
	   if($("#coruses-container").hasClass("products-list"))
	    ...
	    $.ajax({
		        url: '<?php echo e(url(App("urlLang")."categories/reload-products")); ?>',
		        data: {
		        ...
		        success: function( data ) {
		        	...
		        	$("#search_total").html(result[2]);
	 				 ...
	 				 $("#start_at").val("<?php echo e($start_at); ?>");
	...
	$("#new-products_grid").click(function(){
	   ...
	   if($("#coruses-container").hasClass("products-list"))
		...
		var obj=$(this);
		$.ajax({
		        url: '<?php echo e(url(App("urlLang")."categories/productsmore")); ?>',
		        data: {
	                'start_at': start_at,
                    'catIds': JSON.stringify(catIds),
                    'types': JSON.stringify(types),
