...
class forumController extends Controller
{
    ...
    public function getIndex(){ 
		...
		return view('teachers.forum.index',array(
	...
	public function getSearchresults(Request $request)
    {
        ...
        $courseQuestions = $courseQuestions->whereIn("user_id",$student_ids);
        $courseQuestions = $courseQuestions->distinct()->get(["course_questions.*"]);
		
        
		$view =  view('teachers.forum._search_results',	array(
   ...
   public function getEdit($id){
       ...
       foreach(Auth::guard("teachers")->user()->unreadnotifications as $notification){
	    ...
	    return view("teachers.forum.edit",array(
   ...
   public function postEdit(Request $request,$id){
       ...
       $this->saveReplies($request,$courseQuestion);
		...
		$adminhistory->admin_id=Auth::guard("teachers")->user()->id; 
		...
		return redirect("teachers/forum/edit/".$id);
   }
    ...
    public function saveReplies($request,$courseQuestion){
        ...
        if(!empty($replies)){
            foreach($replies as $reply){
			    ...
			    $courseQuestionTmp->user_id = $reply["user_id"];
			    $courseQuestionTmp->type = 2;
