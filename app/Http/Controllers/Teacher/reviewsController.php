...
class reviewsController extends Controller
{
    ...
    public function getIndex(){
		...
		return view('teachers.reviews.index',array(
	...
	public function reply(Request $request){
		$id=$request->id;
		return view('teacher.reviews.create',array(
      		"id"=>$id
		));
	}
	public function createreply(Request $request){
		...
		DB::table('comment_raiting')->insert(
			['rating_id' => $request->rating_id, 'comment' => $request->comment, 'type' => 1, 'user_id' => Auth::guard("teachers")->user()->id, 'date_create' => date('Y-m-d H:i:s')]
		);
		return view('teacher.reviews.create',array(
	...
	public function getSearchresults(Request $request)
    {
		...
		return view('teacher.reviews._search_results',	array(
        			'reviews'=>$reviews
		)); 
