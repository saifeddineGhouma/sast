...
function formFilter(){
	...
	$.ajax({
		...
		data: {year: '{{$year}}',course_id: '{{$course_id}}',
            start: current,length: length, code: '{{$code}}'
        },
		beforeSend: function(){
            $('#graduates_content').after("<img src='{{asset('assets/admin/img/input-spinner.gif')}}' width='20' />");
//			 $("#content_loading").modal("show");
      ...
      console.log(result);
		...
		complete: function(){
            $('#graduates_content').parent().children('img').remove();
