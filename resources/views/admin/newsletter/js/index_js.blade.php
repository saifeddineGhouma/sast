...
$('.deletenewsletter').on('click', deleteRecord);
	function deleteRecord(e) {
		...
		bootbox.confirm({
			...
			message: "Are you sure to delete this newsletter? This operation is irreversible.",
		    callback: function(result) {
				if (result == true) {
		            $.ajax({
		                url: "{{url('admin/newsletter/delete/')}}" + '/' + id,
		                type: 'POST',
		                headers: {
...
function swap(state,field,this1){
	...
	$.ajax({
		url: '{{ url("/admin/newsletter/updatestateajax") }}',					
		type:  'POST',
