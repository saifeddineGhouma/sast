...
function deleteRecord(e) {
	...
	bootbox.confirm({
			title: "Delete News",
			message: "Are you sure to delete this News? This operation is irreversible.",
...
function formFilter(e){
	...
	$.ajax({
		url: '{{ url('/admin/news/searchresults/') }}' ,
		type: "GET",
		data: data,
		beforeSend: function(){
		...
		success: function(result){			    	 					
			$("#newsChildList").html(result);
			$('.deletenews').on('click', deleteRecord); 
...
function swap(state,field,this1){
	...
	$.ajax({
		url: '{{ url("/admin/news/updatestateajax") }}',					
		type:  'POST',
