...
$('.deletetemplate').on('click', deleteRecord);
	function deleteRecord(e) {
		...
		bootbox.confirm({
			...
			message: "Are you sure to delete this template? This operation is irreversible.",
		    callback: function(result) {
				if (result == true) {
		            $.ajax({
		                url: "{{url('admin/emailtemplates/delete/')}}" + '/' + id,
		                type: 'POST',
		                headers: {
...
function swap(state,field,this1){
	...
	$.ajax({
		url: '{{ url("/admin/emailtemplates/updatestateajax") }}',					
		type:  'POST',
