...
$('.deleteteacher').on('click', deleteRecord);
	function deleteRecord(e) {
		...
		bootbox.confirm({
			...
			message: "Are you sure to delete this teacher? This operation is irreversible.",
		    callback: function(result) {
				if (result == true) {
		            $.ajax({
		                url: "{{url('admin/teachers/delete/')}}" + '/' + id,
		                type: 'POST',
		                headers: {
		                ...
		                error: function( data ) {
		                    if ( data.status === 422 ) {
		                        toastr.error('Cannot delete the tag');
		                    }
		                },
				        complete: function(){
