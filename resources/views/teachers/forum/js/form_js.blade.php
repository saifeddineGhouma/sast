...
function addReply(){
    ...
    lastRow.append('<tr id="reply-row'+id+'" data-id="'+id+'">'+
        '<td>'+
            '{{ Auth::guard("teachers")->user()->username }}<input type="hidden" name="replies['+id+'][user_id]" value="{{ Auth::guard("teachers")->user()->id }}" />'+
        '</td>'+
        '<td>'+
            '<textarea cols="60" required  name="replies['+id+'][discussion]" class="form-control"></textarea>'+
        '</td>'+
        '<td>'+
